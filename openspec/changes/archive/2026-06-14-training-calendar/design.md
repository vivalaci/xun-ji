...
### D3：嵌入首页（曲线页）顶部，不新建页面
> 注：初版曾嵌入「训练」列表页顶部；data-pagination 实施时按用户要求移至「首页」（曲线页）顶部，并将该 tab 由"曲线"更名为"首页"。日历置于首页更符合"打开即纵览"。
- 日历区在首页 `<view>` 顶部、时间范围切换之上；下接曲线卡片。
- 数据复用首页已加载的 `workouts`（`onShow`/`refresh` 已拉取，data-pagination 后为全量）。
- 选中日详情：以"选中态 + 日历下方一段详情卡"呈现（不弹层），点条目 `wx.navigateTo` 到 `pages/workout/edit`。
- **被否方案**：独立日历页 / 第 5 tab —— 省一跳、与曲线同屏对照更直观。
