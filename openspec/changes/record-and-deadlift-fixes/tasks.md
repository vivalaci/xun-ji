...
- [ ] 4.3 模拟器/真机走查：① 模板只填一项保存重开数据全在；② 组重量只显示整数或 .5（历史 44.9 显示 45）、lb 录入保存为 0.5 倍数、切换往返稳定、体重明细仍 0.1；③ 硬拉曲线计入罗马尼亚/直腿硬拉、当日多变式取最大、点击进硬拉详情且详情同样聚合（只做过 rdl 时详情非空、历史标注变式）

## 5. 文档

- [x] 5.1 `docs/usermanual.md`：硬拉曲线含罗马尼亚/直腿硬拉、训练组重量统一显示到 0.5（lb 录入同样取整）、保存保留全部动作说明（已同步 `config/manual.js`）
- [ ] 5.2 README 进度 + docs/00 阶段表（归档时，迭代十）
