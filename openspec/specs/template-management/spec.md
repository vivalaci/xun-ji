...
训练模板的查看、重命名、增删，及模板内动作的编辑。模板存于 `workout_templates`，动作仅存 `exerciseId`（力量预设动作另带可缺省的目标组次 `targetSets`/`repLow`/`repHigh`），名称展示时从动作库按 id 取。模板带可缺省的 `group` 字段（预设分三分化/二分化/有氧，用户自建归"我的模板"），列表与选模板界面按组分节展示。预设为 App 托管，经 `user_prefs.presetVersion` 版本重刷升级。
## Requirements
### Requirement: 模板列表管理
